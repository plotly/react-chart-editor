...
import { isTooLight } from "@workspace/utils/color";
...
const SymbolSelector = React.createClass({
  ...
  renderSymbolList() {
    ...
    if (this.props.is3D) {
      symbolOptions = symbolOptions.filter(option => {
  ...
  renderActiveOption() {
    const { symbolColor, borderColor } = this.props;
    const currentlyActive = this.props.activeOption;
    const allSymbols = this.renderSymbolList();

    const currentSymbol = allSymbols.find(
      symbol => symbol.value === currentlyActive
    );
  ...
  renderOptions() {
    const { symbolColor, borderColor } = this.props;
    const allOptions = this.renderSymbolList();
  ...
  render() {
    ...
    const subMenuDisplay = classnames(["symbol-selector__menu"], {
      "+visible": isOpen,
      "+hidden": !isOpen,
    });

    const backgroundColor = isTooLight(this.props.symbolColor)
      ? "#bec8d9"
      : "white";
    ...
    return (
      <div>
        ...
        <div className={subMenuDisplay} style={backgroundStyle}>
          {this.renderOptions()}
        </div>
      </div>
    );
